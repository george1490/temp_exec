...
namespace stdexec {
  ...
  namespace __detail {
    ...
    struct __get_data {
      template <class _Data, typename... T>
      STDEXEC_ATTRIBUTE((always_inline))
      _Data&&
        operator()(__ignore, _Data&& __data, T&&...) const noexcept {
  ...
  namespace {
	...
	struct DefaultDescriptor {
		static constexpr Descriptor value() {
			return Descriptor();
		}
	};
	template <typename Descriptor, auto DescriptorFn = DefaultDescriptor<Descriptor>::value>
	inline constexpr auto __descriptor_fn_v = DescriptorFn;
...
// template <class _Tag, class _Data, class... _Child>
// struct __descriptor {
    // static inline constexpr auto __descriptor_fn() {
        // return __descriptor_fn_v<__detail::__desc<_Tag, _Data, _Child...>>;
    // }
// };

// template <class _Tag, class _Data, class... _Child>
// using __descriptor_fn = typename __descriptor<_Tag, _Data, _Child...>::__descriptor_fn;
...
#  define STDEXEC_SEXPR_DESCRIPTOR(_Tag, _Data, _Child)                                            \
  ...
  struct __uncurry_<_Fn, _Tp, std::enable_if_t<__mvalid<tag_of_t, _Tp> && __mvalid<__detail::__sexpr_uncurry, _Tp, _Fn>>> {
  ...
  namespace __detail {
    ...
    concept __connectable =
      is_callable_v<__impl_of<_Sexpr>, __copy_cvref_fn<_Sexpr>, __connect_fn<_Sexpr, _Receiver>>
...
//   }
		...
		friend void tag_invoke(const set_value_t&, __t&& __self, _Args&&... __args) noexcept {
		...
		friend void tag_invoke(const set_error_t&, __t&& __self, _Error&& __err) noexcept {
		...
		friend void tag_invoke(const set_stopped_t&, __t&& __self) noexcept {
        ...
        template <typename _Self, std::enable_if_t<std::is_same_v<__t, _Self>, int> = 0>
		friend auto tag_invoke(const get_env_t&, const _Self& __self) noexcept -> __env_type_t<_Self, __tag_t, _Idx, _Sexpr, _Receiver> {
...
//     return {};
    ...
    struct __op_state : __op_base<_Sexpr, _Receiver> {
      ...
      __op_state(_Sexpr&& __sexpr, _Receiver __rcvr) noexcept(
        ...
        && is_nothrow_callable_v<__sexpr_apply_t, _Sexpr&&, __connect_fn<_Sexpr, _Receiver>>)
	...
	friend void tag_invoke(const start_t&, __op_state& __self) noexcept {
    ...
    //   }
      ...
      void
        ...
        if constexpr (std::is_invocable_v<_CompleteFn, _Index, __op_state&, _Tag2, _Args...>) {
			// is_callable_v
      ...
      []<class _Fn>(_Fn __fn) noexcept {
        return
          [__fn = std::move(__fn)]<class... _Rest>(auto&&, _Rest&&... __rest) noexcept(
            is_nothrow_callable_v<const _Fn&, _Rest...>) -> __call_result_t<const _Fn&, _Rest...> {
    ...
    constexpr auto
      ...
      return [__captures3 = std::make_tuple(std::forward<_Captures>(__captures2)...)]<class _Cvref, class _Fun>(
    //   return [... __captures3 = static_cast<_Captures&&>(__captures2)]<class _Cvref, class _Fun>(
               _Cvref, _Fun&& __fun) mutable                                           //
             noexcept(is_nothrow_callable_v<_Fun, _Tag, __minvoke<_Cvref, _Captures>...>) //
            ...
            //    requires __callable<_Fun, _Tag, __minvoke<_Cvref, _Captures>...>
      {
		...
		return std::apply([&__fun](auto&&... __captures3) -> __call_result_t<_Fun, _Tag, __minvoke<_Cvref, _Captures>...> {
			return static_cast<_Fun&&>(__fun)(_Tag(), const_cast<__minvoke<_Cvref, decltype(__captures3)>&&>(__captures3)...);
		}, std::move(__captures3));
        // return static_cast<_Fun&&>(__fun)(
        //   _Tag(), const_cast<__minvoke<_Cvref, decltype(__captures3)>&&>(__captures3)...);
      };
    }

template <class _Sender, typename = void>
struct __non_dependent_senderr : std::false_type {};

template <class _Sender>
struct __non_dependent_senderr<_Sender, std::void_t<
    typename _Sender::completion_signatures,
    decltype(_Sender::__is_non_dependent())
>> : std::conditional_t<
    std::is_same_v<decltype(_Sender::__is_non_dependent()), bool>,
    std::bool_constant<_Sender::__is_non_dependent()>,
    std::true_type
> {};

template <class _Sender>
inline constexpr bool __non_dependent_sender = __non_dependent_senderr<_Sender>::value;
...
struct __is_non_dependent_sexprr : std::false_type {};

template <class _Tag, class _Child, class... _Rest>
struct __is_non_dependent_sexprr<_Tag, _Child, _Rest...>
    : std::conditional_t<
        !std::is_same_v<typename __sexpr_impl<_Tag>::is_dependent, void>,
        std::conjunction<
            std::bool_constant<__non_dependent_sender<_Child>>,
            __is_non_dependent_sexprr<_Tag, _Rest...>
        >,
        std::false_type
    > {};

template <class _Tag, class... _Child>
inline constexpr bool __is_non_dependent_sexpr = __is_non_dependent_sexprr<_Tag, _Child...>::value;
...
template <std::size_t _Idx, class _Descriptor, typename = void>
struct __in_rangee : std::false_type {};

template <std::size_t _Idx, class _Descriptor>
struct __in_rangee<_Idx, _Descriptor
> : std::integral_constant<bool, (_Idx < sizeof(__minvoke<_Descriptor, __q<__tuple_size_t>>))> {};

template <std::size_t _Idx, class _Descriptor>
inline constexpr bool __in_range = __in_rangee<_Idx, _Descriptor>::value;
...
template <typename Derived, typename Base>
inline constexpr bool derived_fromf = derived_from_impl<Derived, Base>::value;

template <typename T, typename U>
inline constexpr bool convertible = std::is_convertible_v<T const volatile*, U const volatile*>;

template <typename T, typename U>
inline constexpr bool derived = std::is_base_of_v<U, T> && convertible<T, U>;
...
//       = __detail::__same_as<_Tp, _Up> && __detail::__same_as<_Up, _Tp>;
	...
	template <typename _Self, std::enable_if_t<std::is_same_v<__sexpr, _Self> && std::is_same_v<_Self, __sexpr>, int> = 0>
	friend auto tag_invoke(const get_env_t&, const _Self& __self) noexcept -> __result_of<__sexpr_apply, const _Self&, __get_attrs_fn<__tag_t>> {
    ...
    template <typename _Self, typename _Env, std::enable_if_t<std::is_same_v<std::decay_t<_Self>, __sexpr>, int> = 0>
	friend auto tag_invoke(const get_completion_signatures_t&, _Self&& __self, _Env&& __env) noexcept
      -> __msecond<
        __if_c<std::is_same_v<std::decay_t<_Self>, __sexpr>>,
	...
	template <typename _Self, typename _Receiver, std::enable_if_t<std::is_same_v<std::decay_t<_Self>, __sexpr>, int> = 0>
	friend auto tag_invoke(const connect_t&, _Self&& __self, _Receiver&& __rcvr) 
...
//   concept __decays_to_derived_from = derived_from<__decay_t<_Ty>, _Up>;
	...
	template <std::size_t Idx, typename _Self, std::enable_if_t<derived_fromf<std::decay_t<_Self>, __sexpr> && __detail::__in_range<Idx, __desc_t>>>
	friend auto get(_Self&& __self) noexcept -> decltype(auto) {
		if constexpr (Idx == 0) {
			return __tag_t();
		} else {
			return __self.__impl_(__copy_cvref_fn<_Self>(), __nth_pack_element<Idx>);
		}
	}
  ...
  template <class _Tag, class _Data, class... _Child>
  __sexpr(_Tag, _Data, _Child...) -> __sexpr<stdexec::__descriptor_fn_v<stdexec::__detail::__desc<_Tag, _Data, _Child...>>>;


  template <class _Tag, class _Data, class... _Child>
  using __sexpr_t = __sexpr<stdexec::__descriptor_fn_v<stdexec::__detail::__desc<_Tag, _Data, _Child...>>>;
  ...
  template <class _Sender>
  inline constexpr bool sender_exprr = //
    __mvalid<tag_of_t, _Sender>;

  template <class _Sender, class _Tag>
  inline constexpr bool sender_expr_forr = //
    sender_exprr<_Sender> && std::is_same_v<tag_of_t<_Sender>, _Tag>;

  template <class _Sender>
  inline constexpr bool sender_expr = //
    __mvalid<tag_of_t, _Sender>;

  template <class _Sender, class _Tag>
  concept sender_expr_for = //
    sender_expr<_Sender> && std::is_same_v<tag_of_t<_Sender>, _Tag>;
