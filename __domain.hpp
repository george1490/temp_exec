...
struct __legacy_c11n_fn_impl {
    template <class... _RealArgs>
    auto operator()(
        _RealArgs&&... _args) const
        -> decltype(__make_dispatcher<__legacy_c11n_for<_Tag>, __none_such, _RealArgs...>(
            std::forward<_RealArgs>(_args)...))
    {
        return __make_dispatcher<__legacy_c11n_for<_Tag>, __none_such, _RealArgs...>(
            std::forward<_RealArgs>(_args)...);
    }
};

// template <class _Tag, class... _Args>
// using __legacy_c11n_fnn = __legacy_c11n_fn_impl<_Tag, _Args...>;

    template <class _Tag, class... _Args>
    using __legacy_c11n_fnn = //
      __make_dispatcher<__legacy_c11n_for<_Tag>, __none_such, _Args...>;

// __has_legacy_c11n
template <class _Tag, class... _Args>
struct __has_legacy_c11ne {
private:
    template <class... _RealArgs>
    static auto test(int) -> decltype(
        std::declval<__legacy_c11n_fnn<_Tag, _RealArgs...>>()(
            std::declval<_RealArgs>()...),
        std::true_type{});
...
public:
    static constexpr bool value = decltype(test<_Args...>(0))::value;
};

template <class _Tag, class... _Args>
constexpr bool __has_legacy_c11nnnnnn = __has_legacy_c11ne<_Tag, _Args...>::value;
    ...
    //   std::is_invocable_v<__legacy_c11n_fn<_Tag, _Args...>, _Args...>;

    // template <class _Tag, class... _Args>
    // inline constexpr bool __has_legacy_c11nn = //
    //   std::is_invocable_v<__legacy_c11n_fn<_Tag, _Args...>, _Args...>;

    struct __legacy_customization {
      template <class _Tag, class _Data, class... _Children, std::enable_if_t<__has_legacy_c11nnnnnn<_Tag, _Data, _Children...>, int> = 0>
        // requires __has_legacy_c11nnnnnn<_Tag, _Data, _Children...>
...
constexpr auto __has_transform_sender_impl(int) -> decltype(
    std::declval<_DomainOrTag>().transform_sender(
        std::declval<_Sender&&>(), std::declval<const _Env&>()...), bool{}) {
    return true;
}

template <class _DomainOrTag, class _Sender, class... _Env>
constexpr bool __has_transform_sender_impl(...) {
    return false;
}

template <class _DomainOrTag, class _Sender, class... _Env>
inline constexpr bool __has_transform_sender = __has_transform_sender_impl<_DomainOrTag, _Sender, _Env...>(0);
    ...
    inline constexpr bool __has_apply_sender = requires(_DomainOrTag __tag, _Args&&... __args) {
      __tag.apply_sender(static_cast<_Args&&>(__args)...);
    };

    // template <class _Sender>
    // constexpr bool __is_nothrow_transform_sender() {
    //   if constexpr (std::is_invocable_v<__sexpr_apply_t, _Sender, __domain::__legacy_customization>) {
    ...
    template <class _Sender, std::enable_if_t<std::is_invocable_v<__sexpr_apply_t, _Sender, __domain::__legacy_customization>, int> = 0>
    constexpr bool __is_nothrow_transform_sender() {
    //   if constexpr (std::is_invocable_v<__sexpr_apply_t, _Sender, __domain::__legacy_customization>) {
        return __nothrow_callable<__sexpr_apply_t, _Sender, __domain::__legacy_customization>;
    //   } else if constexpr (__domain::__has_default_transform_sender<_Sender>) {
    ...
    template <class _Sender, std::enable_if_t<std::is_nothrow_constructible_v<_Sender, _Sender>, int> = 0>
    constexpr bool __is_nothrow_transform_sender() {
    	// if constexpr (__domain::__has_default_transform_sender<_Sender>) {
        // return noexcept(tag_of_t<_Sender>().transform_sender(__declval<_Sender>()));
    //   } else {
        return __nothrow_constructible_from<_Sender, _Sender>;
    //   }
    ...
    template <class _Sender, std::enable_if_t<!std::is_invocable_v<__sexpr_apply_t, _Sender, __domain::__legacy_customization> && !std::is_nothrow_constructible_v<_Sender, _Sender>, int> = 0>
    constexpr bool __is_nothrow_transform_sender() {
    	// if constexpr (__domain::__has_default_transform_sender<_Sender>) {
        return noexcept(tag_of_t<_Sender>().transform_sender(__declval<_Sender>()));
    //   } else {
    ...
    template <class _Sender, class _Env, std::enable_if_t<!std::is_nothrow_constructible_v<_Sender, _Sender>, int> = 0>
  ...
  struct default_domain {
    ...
    // template <class _Tag, class _Sender, class... _Args, std::enable_if_t<!__domain::__has_legacy_c11nn<_Tag, _Sender, _Args...>, int>>
    // //   requires __domain::__has_legacy_c11n<_Tag, _Sender, _Args...>
    //         // || __domain::__has_apply_sender<_Tag, _Sender, _Args...>
    // STDEXEC_ATTRIBUTE((always_inline))
    // decltype(auto)
  ...
  inline constexpr struct __get_early_domain_t {
    ...
    auto operator()(const _Sender&, _Default __def = {}) const noexcept {
      if constexpr (std::is_invocable_v<get_domain_t, env_of_t<_Sender>>) {
  ...
  inline constexpr struct __get_late_domain_t {
    ...
    auto operator()(const _Sender& __sndr, const _Env& __env) const noexcept {
      ...
      } else if constexpr (std::is_invocable_v<get_domain_t, const _Env&>) {
        return get_domain(__env);
      } else if constexpr (std::is_invocable_v<__composed<get_domain_t, get_scheduler_t>, const _Env&>) {
