...
namespace stdexec {
  ...
  inline constexpr struct __compose_t {
    template <class _Fun0, class _Fun1>
    STDEXEC_ATTRIBUTE((always_inline))
    __composed<_Fun0, _Fun1>
      operator()(_Fun0 __fun0, _Fun1 __fun1) const {
  ...
  namespace __invoke_ {
    ...
    struct __funobj {
      ...
      STDEXEC_ATTRIBUTE((always_inline))
      constexpr auto
        operator()(_Fun&& __fun, _Args&&... __args) const
  ...
  template <class _Fn, class _Tup>
  inline constexpr bool __applicable = __mvalid<__apply_::__result_t, _Fn, _Tup>;
  ...
  struct __field {
    STDEXEC_ATTRIBUTE((always_inline))
    _Ty
      operator()(_Tag) const noexcept(std::is_nothrow_constructible_v<std::decay_t<const _Ty&>, _Ty>) {
      return __t_;
    }
  ...
  struct __mkfield_ {
    template <class _Ty>
    STDEXEC_ATTRIBUTE((always_inline))
    __field<_Tag, __decay_t<_Ty>>
      operator()(_Ty&& __ty) const noexcept(std::is_nothrow_constructible_v<std::decay_t<_Ty>, _Ty>) {
...
inline constexpr bool is_nothrow_tag_invocable_v =
    noexcept(try_tag_invoke<CPO, Args...>(0));
...
////////////////////////////////////////////////////////////////
  ...
  namespace __tag_invoke {
	...
	template <
		...
		typename = tag_invoke_result_t<CPO, Args...>
	>
	yes_type try_tag_invoke(int) noexcept(noexcept(tag_invoke(std::declval<CPO&&>(), std::declval<Args&&>()...)));
	...
	template <typename CPO, typename... Args>
	inline constexpr bool is_tag_invocable_v = (sizeof(try_tag_invoke<CPO, Args...>(0)) == sizeof(yes_type));

template <typename Tag, typename... Args>
inline constexpr bool is_nothrow_tag_invocable_v = noexcept(try_tag_invoke<Tag, Args...>(0));

template <typename Tag, typename... Args>
using is_nothrow_tag_invocable = std::bool_constant<is_nothrow_tag_invocable_v<Tag, Args...>>;
	...
	template <typename Tag, typename... Args>
	inline constexpr bool tag_invocable =
		    (sizeof(try_tag_invoke<Tag, Args...>(0)) ==
     sizeof(yes_type));
    ...
    struct tag_invoke_t {
      template <class _Tag, class... _Args, std::enable_if_t<tag_invocable<_Tag, _Args...>, int> = 0>
        // requires tag_invocable<_Tag, _Args...>
      STDEXEC_ATTRIBUTE((always_inline))
      constexpr auto
        operator()(_Tag __tag, _Args&&... __args) const
        noexcept(is_nothrow_tag_invocable_v<_Tag, _Args...>) -> tag_invoke_result_t<_Tag, _Args...> {
  ...
  using __tag_invoke::is_nothrow_tag_invocable;
  using __tag_invoke::is_nothrow_tag_invocable_v;
} // namespace stdexec
